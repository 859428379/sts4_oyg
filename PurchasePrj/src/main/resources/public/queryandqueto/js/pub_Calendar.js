...
function fPopUpCalendarDlg(ctrlobj)   //弹出日历
{
	...
	retval = window.showModalDialog("../pub_CalendarDlg.asp", "", "dialogWidth:172px; dialogHeight:210px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
	if( retval != null ){
...
function fPopUpMonthDlg(ctrlobj)   //弹出日历 只返回月份 
{
	showx = event.screenX - event.offsetX - 4 - 210 ; // + deltaX;
	showy = event.screenY - event.offsetY + 18; // + deltaY;
	newWINwidth = 210 + 4 + 18;
...
write("<B style='color:"+gcBG+";cursor:hand; font-size:11px' onclick='fSetDate(giYear,giMonth,giDay)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=gcBG'>选择: "+giYear+"/"+giMonth+"/"+giDay+"</B>");
write("</td></tr></table>");
write("</TD></TR>");
write("</TABLE></Div>");
}
